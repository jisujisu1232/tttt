...
resource "aws_security_group" "app_to_db" {
  ...
  name        = "${var.name}-db-for-app-sg"
  description = "${var.name} Security Group For App"
  vpc_id      = "${var.vpc_id}"
}
...
resource "aws_security_group_rule" "admin_to_db" {
  count             = length(var.vpc_security_group_ids)==0 ? 1 : 0
  description       = "${var.name} DB for Admin"
...
resource "aws_security_group_rule" "app_to_db" {
  count             = length(var.vpc_security_group_ids)==0 ? 1 : 0
  description       = "${var.name} DB for App"
  security_group_id = "${aws_security_group.db[0].id}"
  source_security_group_id       = "${aws_security_group.app_to_db[0].id}"
  protocol          = "tcp"
  from_port         = "${var.port}"
  to_port           = "${var.port}"
  type              = "ingress"
...
resource "aws_db_instance" "this" {
  ...
  vpc_security_group_ids = length(var.vpc_security_group_ids)==0 ? [aws_security_group.db[0].id] : var.vpc_security_group_ids
